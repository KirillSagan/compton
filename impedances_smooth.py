...
class Impedance(Element):
    ...
    def track(self, bunch):
        ...
        # Make fft lambda z
        lambda_z_fft = np.fft.rfft(lambda_z)
        lambda_z_wake = norm.pdf(z,0,self.sigma_z_wake)
        lambda_z_wake_fft = np.fft.rfft(lambda_z_wake)

        # Make freqs array and find max freq
        delta_freq = 1/(self.times.max()-self.times.min())
        freq_max = c/(self.sigma_z_wake)/2
        n_freq_max = int(freq_max/delta_freq)
        self.freqs = np.fft.fftfreq(len(z),self.times[1]-self.times[0])

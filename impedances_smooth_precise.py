...
from PyHEADTAIL.impedances.impedance_kicks_smooth_precise import *
...
class Impedance(Element):
    ...
    def __init__(self, slicer, *wake_sources, sigma_z_wake = 1e-3, dz=5e-6, z_max=50e-3):
    ...
    def track(self, bunch):
        ...
        # Get bunch lambda z
        z = slice_set.z_centers
        lambda_z = -1*slice_set.lambda_z(z)/e/bunch.intensity*bunch.macroparticlenumber
        self.times = slice_set.convert_to_time(z)
        
        # interpolation z  
        z_interp = np.arange(-z_max, z_max, dz)
        times_interp = z_interp/c
        lambda_z_interp = interp1d(z,lambda_z, bounds_error=False, fill_value= 0.0)(z_interp) 
        
        # Make fft lambda z
        lambda_z_fft = np.fft.rfft(lambda_z_interp)
        lambda_z_wake = norm.pdf(z_interp, 0, self.sigma_z_wake)
        lambda_z_wake_fft = np.fft.rfft(lambda_z_wake)

        # Make freqs array and find max freq
        delta_freq = c/(z_max*2)
        freq_max = c/(self.sigma_z_wake)/2
        n_freq_max = int(freq_max/delta_freq)
        self.freqs = np.fft.fftfreq(len(z_interp),dz/c)
        ...
        for kick in self.wake_kicks:
            kick.apply(bunch, self.times, times_interp, lambda_z_fft, lambda_z_wake_fft,
...
class ImpedanceTable(WakeSource):
    ...
    def __init__(self, wake_file, wake_file_columns, n_turns_wake=1,
        ...
        super(ImpedanceTable, self).__init__(*args, **kwargs)
    ...
    def function_transverse(self, wake_component):
        ...
        interpolation_function = interp1d(time, wake_strength, bounds_error=False, fill_value= 0.0)
